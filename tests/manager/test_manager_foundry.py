"""Appendix B sub-project 3g (Task 1): the manager as an ADVISORY foundry domain.
run_foundry_agent dispatches run_manager under an owner-attested {domain:"manager",
ceiling:"A0"} record WITHOUT opening a run or touching the sandbox; the routing
proposal is returned. Unknown/inactive/no-domain -> refused, nothing dispatched."""
...
# A minimal, deterministic routable spec (route() matches name/title/description/keywords).
SPEC = AgentSpec(name="widget", tier="open", category="core", title="Widget",
                 description="builds widgets", keywords=("widget",))


class _NoExecClient:
    """A record source that FORBIDS execution: if the manager dispatch ever opens
    a run or touches the sandbox, these raise -> the test fails."""
    def __init__(self, rec): self._rec = rec
    def foundry_agent(self, aid): return self._rec
    def open_run(self, *a, **k): raise AssertionError("manager must not open a run")
    def sandbox_execute(self, *a, **k): raise AssertionError("manager must not touch the sandbox")


def test_manager_dispatch_returns_proposal_without_executing():
    c = _NoExecClient({"activation_state": "active", "ceiling": "A0", "domain": "manager"})
    out = run_foundry_agent(client=c, store=object(), agent_id="M", task_id="t",
                            demand={"intent": "build a widget"}, specs=[SPEC])
    assert out["ok"] is True and out["agent_id"] == "M" and out["ceiling"] == "A0"
    assert out["result"]["status"] == "proposed"
    assert out["result"]["recommended_agent"] == "widget"     # routed via the injected spec
    assert isinstance(out["result"]["registry"], list)


def test_manager_gap_is_still_a_proposal():
    c = _NoExecClient({"activation_state": "active", "ceiling": "A0", "domain": "manager"})
    out = run_foundry_agent(client=c, store=object(), agent_id="M", task_id="t",
                            demand={"intent": "zzzz nomatch"}, specs=[SPEC])
    assert out["ok"] is True and out["result"]["status"] == "proposed"
    assert out["result"]["recommended_agent"] is None and out["result"]["gap"]


def test_manager_inactive_refused():
    c = _NoExecClient({"activation_state": "quarantined", "ceiling": "A0", "domain": "manager"})
    out = run_foundry_agent(client=c, store=object(), agent_id="M", task_id="t",
                            demand={"intent": "build a widget"}, specs=[SPEC])
...
# ---- served-CP integration (no podman: the manager opens no sandbox run) ----
...
def test_manager_foundry_served_cp():
    server, uds, priv = _serve_owner()
    try:
        client = ControlPlaneClient(uds)
        M = _mint(uds, priv, "mgr", "A0", domain="manager")
        out = run_foundry_agent(client=client, store=object(), agent_id=M, task_id="m1",
                                demand={"intent": "build a widget"}, specs=[SPEC])
        assert out["ok"] is True, out
        assert out["agent_id"] == M and out["ceiling"] == "A0"      # attested identity + A0
        assert out["result"]["status"] == "proposed"
        assert out["result"]["recommended_agent"] == "widget"

        plain = _mint(uds, priv, "plain", "A0")                     # no domain
        ref = run_foundry_agent(client=client, store=object(), agent_id=plain, task_id="m2",
                                demand={"intent": "build a widget"}, specs=[SPEC])
