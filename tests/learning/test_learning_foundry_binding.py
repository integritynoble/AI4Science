"""Appendix B sub-project 3f (Task 1): run_learning_task binds to a foundry
agent_id (the CP derives the ceiling), and foundry_runner dispatches the
`learning` domain to it. Mirrors the research binding from 3e (open-before-compile)."""
from ai4science.harness.agents.learning import agent as learning_mod
from ai4science.harness.agents import foundry_runner as fr

DEMAND = {"topic": "cells", "material": {"m.txt": "mitochondria are the powerhouse"},
          "coverage_points": ["mitochondria"], "min_questions": 1}
...
def test_agent_id_binds_run_and_contract_uses_derived_profile(monkeypatch):
    ...
    monkeypatch.setattr(learning_mod, "run_task", _stub_run_task)
    client = _FakeClient(derived_profile="A1")          # CP derives A1 for this agent
    out = learning_mod.run_learning_task(demand=DEMAND, client=client, store=object(), task_id="t1",
                                         governed=False, planner=object(),
                                         agent_id="L", capability_profile="A0")   # caller asks A0
    assert out["status"] == "stubbed"
    assert client.open_run_calls[0]["agent_id"] == "L"
...
def test_no_agent_id_is_unbound_and_unchanged(monkeypatch):
    ...
    learning_mod.run_learning_task(demand=DEMAND, client=client, store=object(), task_id="t2",
...
def test_foundry_runner_dispatches_learning_bound(monkeypatch):
    calls = []
    def _spy(*, client, store, agent_id, task_id, **kw):
        calls.append({"agent_id": agent_id, "kw": kw}); return {"status": "delivered"}
    monkeypatch.setitem(fr.DOMAIN_SPECS, "learning", fr.DomainEntry(min_profile="A1", run=_spy))
    class _Rec:
        def foundry_agent(self, aid): return {"activation_state": "active", "ceiling": "A1", "domain": "learning"}
    out = fr.run_foundry_agent(client=_Rec(), store=object(), agent_id="L", task_id="t",
                               demand=DEMAND)
    assert out["ok"] is True and out["ceiling"] == "A1"
    assert calls[0]["agent_id"] == "L"                       # ran bound
    assert calls[0]["kw"]["demand"] == DEMAND                # **kw carried the demand


def test_foundry_runner_refuses_learning_below_a1_floor(monkeypatch):
    calls = []
    def _spy(*, client, store, agent_id, task_id, **kw):
        calls.append(agent_id); return {"status": "delivered"}
    monkeypatch.setitem(fr.DOMAIN_SPECS, "learning", fr.DomainEntry(min_profile="A1", run=_spy))
    class _Rec:
        def foundry_agent(self, aid): return {"activation_state": "active", "ceiling": "A0", "domain": "learning"}
    out = fr.run_foundry_agent(client=_Rec(), store=object(), agent_id="L", task_id="t", demand=DEMAND)
