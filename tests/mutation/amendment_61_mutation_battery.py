...
BASELINE = [
    ...
    # tests/test_response_metadata.py — DROPPED (does not exist on this branch).
    # The reference tree listed it as an 8th suite; the response-metadata
    # surface it instrumented was never landed on feat/pwm-qwen-route, so the
    # file is absent and cannot be part of the baseline here. The remaining
    # seven suites were each verified present on this worktree before this
    # list was finalised.
...
# Every mutant ever defined for this battery — active AND inactive. Membership
# is decided MECHANICALLY below (NOT_APPLICABLE): a mutant runs on this branch
# only if its file exists and every one of its hunks (including EXTRA_HUNKS)
# anchors exactly once. Inactive mutants are kept here verbatim so a later
# session can revive them when the missing code lands.
#
# NOTE on `expect_failures`: every mutant that RUNS on this branch has been
# reset to None. The reference's per-mutant counts were measured against a
# different suite set (which included tests/test_response_metadata.py, absent
# here), so every one of them would read as spurious "count drift" against this
# baseline. Inactive mutants keep their original recorded counts for revival.
_ALL_MUTANTS: List[Mutant] = [
    # ---------------------------------------------------------------- Task 2
    Mutant(
        id="mA",
    ...
    Mutant(
        ...
        new="""            elif failure is None:
                yield event""",
        expect_killed=True,
        expect_failures=None,  # zeroed: reference counts were measured against a different suite set (see note above MUTANTS)
    ),
    Mutant(
        id="mT2",
...
""",
    ...
    Mutant(
        ...
        path=R,
        # ANCHOR RE-INDENTED for feat/pwm-qwen-route: the reference's
        # `_route_adapter` is a nested closure (8-space body); on this branch it
        # is a module-level function (4-space body). Only the leading whitespace
        # changed — same statements, same surface, same defect — so this is a
        # re-anchor of the existing mutant, NOT a new one.
    ...
    if True:
        return adapter""",
        expect_killed=True,
        expect_failures=None,  # re-anchored active mutant; count not carried over
    ),
    Mutant(
        id="mT9b",
        ...
        path=R,
        # ANCHOR REWRITTEN for feat/pwm-qwen-route: the source tree's child site
        # read `adapter=_route_adapter(child_backend, child_model, spec),`. On
        # this branch the child-session site reads with the active_* names, so
        # the anchor is updated to this tree's text (id, surface, defect kept).
        old="""            adapter=_route_adapter(active_backend, active_model, spec),""",
        new="""            adapter=adapter_for(active_backend),""",
        expect_killed=True,
    ...
    Mutant(
        ...
        path=R,
        # ANCHOR RE-INDENTED for feat/pwm-qwen-route: `_route_adapter` is
        # module-level here (4-space body) rather than the reference's nested
        # closure (8-space body), so the `return AttestedAdapter(...)` and its
        # continuation shift left by four columns. Same statement, same defect
        # (strict=True → strict=False) — a re-anchor, not a new mutant.
    ...
    Mutant(
        ...
        path=R,
        # ANCHOR REWRITTEN for feat/pwm-qwen-route: the source tree spread the
        # `/model` call over three lines. On this branch the same call site is a
        # single line, so the anchor is updated to this tree's one-line text
        # (id, surface, defect kept).
        old="""                    session.set_brand(_route_adapter(new_backend, new_model, active_spec), new_model, new_backend)""",
        new="""                    session.set_brand(adapter_for(new_backend), new_model, new_backend)""",
        expect_killed=True,
        expect_failures=None,
        note="SURVIVED until 2026-08-18; killed by "
    ...
    Mutant(
        ...
        path=R,
        # ANCHOR RE-INDENTED for feat/pwm-qwen-route: `_route_adapter` is
        # module-level here (4-space body), not the reference's nested closure
        # (8-space body). The two `required_*` assignments only lose four columns
        # of leading whitespace — same statements, same defect (route read from
        # the caller, not the spec) — so this is a re-anchor, not a new mutant.
    ...
    Mutant(
        id="mT9f",
        surface="REPL orchestration-chain fallback call site",
        defect="a strict function whose primary backend is unavailable is "
               "silently re-branded onto another backend during auto-heal",
        path=R,
        # NEW on feat/pwm-qwen-route: this auto-heal fallback set_brand site does
        # not exist in the reference wiring, so it gets its own mutant. Same
        # unwrap-the-guard defect as mT9b/mT9d, but at the orchestration-chain
        # fallback call site rather than the child or /model sites.
        old="""                session.set_brand(_route_adapter(nb, nm, active_spec), nm, nb)""",
        new="""                session.set_brand(adapter_for(nb), nm, nb)""",
        expect_killed=True,
        expect_failures=None,
        provenance="new",
    ),
...
#: Mutants that CANNOT run on feat/pwm-qwen-route, decided MECHANICALLY: for
#: each mutant (including its EXTRA_HUNKS) the target path was resolved in this
#: worktree and every hunk's anchor was counted. A mutant is dropped here iff
#: its file is missing OR `text.count(old) != 1` for any hunk. Every reason
#: names the file and says whether the file was missing or the anchor text was
#: absent (with the anchor's first line). Dropped mutants are NOT deleted — they
#: stay in `_ALL_MUTANTS` (see `INACTIVE_MUTANTS`) and are printed in `--list`
#: and the run summary exactly the way `UNRECOVERED` is, so they stay VISIBLE.
#: Most of these target Amendment 61 code from Tasks 2/4 that was never landed
#: on this branch (the openai/proxy/gateway metadata surfaces, the planner
#: strict wiring, the imaging spec). NOTE: mT9/mT9c/mT9e were once dropped here
#: on the same "indented differently" reasoning — the _route_adapter body sits
#: at module level (4-space) on this branch, not the reference's nested closure
#: (8-space) — but that is a re-anchor, not an inapplicability: their statements
#: exist verbatim here. They have been re-indented and returned to the ACTIVE
#: MUTANTS list, so they are no longer dropped.
NOT_APPLICABLE = {
    'mA': "ai4science/harness/adapters/openai.py: anchor text absent on this branch (first line: '            for ch in chunks:')",
    'mB1': "ai4science/harness/adapters/openai.py: anchor text absent on this branch (first line: '        if phase is _MetaPhase.COLLECTING:')",
    'mB2': "ai4science/harness/adapters/openai.py: anchor text absent on this branch (first line: '        except Exception:')",
    'mC1': 'ai4science/harness/adapters/proxy.py: anchor text absent on this branch (first line: \'                        if kind == "meta":\')',
    'mC2': "ai4science/harness/adapters/proxy.py: anchor text absent on this branch (first line: '                        if phase is _ProxyMetaPhase.WAITING:')",
    'mC3': "ai4science/harness/adapters/proxy.py: anchor text absent on this branch (first line: '                        except (TypeError, ValueError) as exc:')",
    'mE1': 'ai4science/harness/adapters/proxy.py: anchor text absent on this branch (first line: \'                        if kind == "bill":\')',
    'mF1': 'ai4science/harness/adapters/openai.py: anchor text absent on this branch (first line: \'                observed_model=observed["observed_model"],\')',
    'mF2': 'ai4science/llm/openai_compat.py: anchor text absent on this branch (first line: \'        observed_model=response.get("model"),\')',
    'mG1': 'ai4science/harness/adapters/proxy.py: anchor text absent on this branch (first line: \'                            yield replace(ev, transport="proxy")\')',
    'mM1': "ai4science/harness/adapters/openai.py: anchor text absent on this branch (first line: '            from ai4science.harness import transport')",
    'mD1': "ai4science/harness/llm_gateway.py: anchor text absent on this branch (first line: '                if phase is _GatewayMetaPhase.WAITING:')",
    'mD1-mid': "ai4science/harness/llm_gateway.py: anchor text absent on this branch (first line: '                if phase is _GatewayMetaPhase.WAITING:')",
    'mT6': 'ai4science/harness/repl.py: anchor text absent on this branch (first line: \'            if getattr(active_spec, "strict_route", False):\')',
    'mT8': "ai4science/harness/agents/imaging/llm/planner.py: anchor text absent on this branch (first line: '        except Exception:')",
    # mT9 / mT9c / mT9e were dropped here as "anchor text absent"; that was a
    # false drop. They failed ONLY on leading whitespace — the reference's
    # `_route_adapter` is a nested closure (8-space body) while on this branch it
    # is a module-level function (4-space body). Their anchors have been
    # re-indented to this tree's text and moved back into the ACTIVE list above,
    # so they are no longer listed here.
    'mP1': "ai4science/harness/agents/imaging/llm/planner.py: anchor text absent on this branch (first line: '        self.strict = bool(strict)')",
    'mP2': 'ai4science/harness/agents/imaging/llm/planner.py: anchor text absent on this branch (first line: \'                 reasoning: str = "medium", solvers=None, strict: bool = False):\')',
    'mP4': "ai4science/harness/agents/specs/imaging.py: anchor text absent on this branch (first line: '    strict_route=True,')",
}

#: The runnable manifest: every mutant whose anchors all match exactly once on
#: this branch. `main()` iterates this, exactly as the reference iterated its
#: `MUTANTS`.
MUTANTS: List[Mutant] = [m for m in _ALL_MUTANTS if m.id not in NOT_APPLICABLE]

#: The dropped mutants, kept whole for a later session to revive. NOT deleted.
INACTIVE_MUTANTS: List[Mutant] = [m for m in _ALL_MUTANTS if m.id in NOT_APPLICABLE]

# Sanity: every dropped id must correspond to a real mutant definition, and the
# split must account for every mutant exactly once.
assert set(NOT_APPLICABLE) <= {m.id for m in _ALL_MUTANTS}
assert len(MUTANTS) + len(INACTIVE_MUTANTS) == len(_ALL_MUTANTS)
...
def main() -> int:
    ...
    if args.list:
        ...
        for mid, why in NOT_APPLICABLE.items():
            print(f"{mid:10s} {'N/A-HERE':9s} {'':14s} {why}")
        return 0

    selected = [m for m in MUTANTS if not args.ids or m.id in args.ids]
    unknown = set(args.ids) - {m.id for m in MUTANTS} - set(NOT_APPLICABLE)
    if unknown:
        print(f"unknown mutant id(s): {sorted(unknown)}")
        return 1
    inactive_requested = set(args.ids) & set(NOT_APPLICABLE)
    if inactive_requested:
        for mid in sorted(inactive_requested):
            print(f"{mid}: NOT APPLICABLE on this branch — {NOT_APPLICABLE[mid]}")
    ...
    try:
        ...
        if NOT_APPLICABLE:
            print(f"not applicable on this branch (dropped mechanically, kept "
                  f"for revival): {', '.join(sorted(NOT_APPLICABLE))}")
            for mid in sorted(NOT_APPLICABLE):
                print(f"    {mid}: {NOT_APPLICABLE[mid]}")
        if mismatches:
