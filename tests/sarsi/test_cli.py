...
def test_agents_never_prints_a_bot_token(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    admin.set_bot_token("sarsi-worker", "8541204756:AA-secret")
    result = runner.invoke(app, ["sarsi", "agents", "--bindings"])
...
def test_ask_reaches_the_named_agent(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "ask", "sarsi-worker", "triage my mail"])
    assert result.exit_code == 0
    assert "sarsi-worker" in result.output
...
def test_ask_records_on_the_same_log_the_bot_writes_to(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "ask", "sarsi-worker", "use the staging host"])
    config = reg.load()
    entries = ownerlog.said(config, config.agents["sarsi-worker"])
...
def test_do_admits_a_directive_this_machine_can_run(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "tidy the report folder"])
    assert result.exit_code == 0
...
def test_do_refuses_and_names_the_missing_tool(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "run the segmentation",
                                 "--tool", "time-machine"])
    assert result.exit_code != 0
...
def test_tasks_lists_what_the_worker_holds(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", "tidy the report folder"])
    result = runner.invoke(app, ["sarsi", "tasks", "sarsi-worker"])
...
def test_tasks_does_not_list_a_refused_directive(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", "run the segmentation",
                        "--tool", "time-machine"])
    result = runner.invoke(app, ["sarsi", "tasks", "sarsi-worker"])
...
def test_do_creates_a_task_with_a_plan(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "tidy the report folder"]).output
    task_id = [w for w in out.split() if w.startswith("tsk_")][0]
    plan = runner.invoke(app, ["sarsi", "plan", "sarsi-worker", task_id]).output
...
def test_a_task_needing_a_permission_waits_for_the_owner(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "read my mail",
...
def test_granting_the_declared_permission_releases_the_task(isolated):
    ...
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "read my mail",
                              "--secret", "mail.read"]).output
    task_id = [w for w in out.split() if w.startswith("tsk_")][0]
    granted = runner.invoke(app, ["sarsi", "grant", "sarsi-worker", task_id,
                                  "read secret mail.read"])
    assert granted.exit_code == 0
    listing = runner.invoke(app, ["sarsi", "tasks", "sarsi-worker"]).output
...
def test_tasks_shows_why_a_task_is_not_running(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", "read my mail", "--secret", "mail.read"])
    result = runner.invoke(app, ["sarsi", "tasks", "sarsi-worker"])
...
def test_run_refuses_a_task_still_awaiting_a_grant(isolated):
    ...
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "read my mail",
                              "--secret", "mail.read"]).output
    result = runner.invoke(app, ["sarsi", "run", "sarsi-worker", _task_id(out)])
    assert result.exit_code != 0
...
def test_operate_reports_what_it_did(isolated, monkeypatch):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "write DONE.md"]).output
    task_id = [w for w in out.split() if w.startswith("tsk_")][0]
    ...
    result = runner.invoke(app, ["sarsi", "operate", "sarsi-worker", task_id, "--passes", "1"])
    assert result.exit_code == 0
...
def test_check_without_a_verifier_never_passes(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "tidy the folder"]).output
    result = runner.invoke(app, ["sarsi", "check", "sarsi-worker", _task_id(out),
...
def test_a_run_needing_a_denied_secret_names_it(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    vault.put(reg.load(), "mail.read", "hunter2")
    out = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "triage my mail",
                              "--secret", "mail.read"]).output
    task_id = [w for w in out.split() if w.startswith("tsk_")][0]
    runner.invoke(app, ["sarsi", "grant", "sarsi-worker", task_id, "read secret mail.read"])
    result = runner.invoke(app, ["sarsi", "run", "sarsi-worker", task_id, "--deny-secrets"])
    assert result.exit_code != 0
...
def test_send_shows_the_whole_body_and_stops(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "send", "sarsi-worker", "--kind", "mail",
                                 "--to", "bob@example.com",
...
def test_send_refused_is_recorded_as_an_outcome(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "send", "sarsi-worker", "--kind", "mail", "--dry-run",
...
def test_ceiling_names_one_agent_without_touching_the_rest(isolated):
    ...
    runner.invoke(app, ["sarsi", "ceiling", "sarsi-worker", "A2"])
    from ai4science.harness.agents.sarsi import registry as reg
    config = reg.load()
    assert config.agents["sarsi-worker"].ceiling == "A2"
...
def test_ceiling_refuses_a_level_the_gate_does_not_know(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "ceiling", "sarsi-worker", "A9"])
...
def _one_task(goal="tidy the report folder"):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", goal])
    from ai4science.harness.agents.sarsi import task as tsk
    config = reg.load()
    return config, config.agents["sarsi-worker"], tsk.all_of(config, config.agents["sarsi-worker"])[0]


def test_cli_stop_frees_the_slot(isolated):
    from ai4science.harness.agents.sarsi import task as tsk
    config, agent, t = _one_task()
    result = runner.invoke(app, ["sarsi", "stop", "sarsi-worker", t.id])
    assert result.exit_code == 0
...
def test_cli_archive_takes_it_off_the_board(isolated):
    from ai4science.harness.agents.sarsi import task as tsk
    config, agent, t = _one_task()
    runner.invoke(app, ["sarsi", "archive", "sarsi-worker", t.id])
...
def test_cli_tasks_can_show_the_archive(isolated):
    ...
    runner.invoke(app, ["sarsi", "archive", "sarsi-worker", t.id])
    result = runner.invoke(app, ["sarsi", "tasks", "sarsi-worker", "--archived"])
    assert "tidy the report folder" in result.output
...
def test_cli_goal_moves_it(isolated):
    from ai4science.harness.agents.sarsi import task as tsk
    config, agent, t = _one_task()
    result = runner.invoke(app, ["sarsi", "goal", "sarsi-worker", t.id, "rebuild the index"])
    assert result.exit_code == 0
...
def test_cli_retry_refuses_without_a_judged_failure(isolated):
    ...
    result = runner.invoke(app, ["sarsi", "retry", "sarsi-worker", t.id])
    assert result.exit_code != 0
...
def test_cli_attention_names_an_ungranted_permission(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", "read my mail",
                        "--secret", "mail.read"])
    result = runner.invoke(app, ["sarsi", "attention"])
    assert "mail.read" in result.output and "sarsi-worker" in result.output
...
def test_cli_enter_asks_when_the_board_is_empty(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "enter", "sarsi-worker"])
    assert result.exit_code == 0 and "?" in result.output


def test_cli_why_answers_from_the_record(isolated):
    _config, _agent, t = _one_task("tidy the report folder")
    result = runner.invoke(app, ["sarsi", "why", "sarsi-worker", t.id])
    assert result.exit_code == 0
    assert "tidy the report folder" in result.output
...
def test_cli_check_can_name_one_phase(isolated):
    ...
    result = runner.invoke(app, ["sarsi", "check", "sarsi-worker", t.id, "--phase", "1",
                                 "--evidence", "x", "--no-model"])
    assert result.exit_code == 0
...
def test_cli_check_rejects_a_phase_that_does_not_exist(isolated):
    _config, _agent, t = _one_task()
    result = runner.invoke(app, ["sarsi", "check", "sarsi-worker", t.id, "--phase", "9",
                                 "--evidence", "x", "--no-model"])
    assert result.exit_code != 0
...
def test_cli_do_can_declare_the_working_directory(isolated, tmp_path):
    ...
    work.mkdir()
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "reconstruct the cube",
                                 "--workdir", str(work)])
    assert result.exit_code == 0
    config = reg.load()
    agent = config.agents["sarsi-worker"]
...
def test_cli_do_refuses_a_working_directory_that_is_not_there(isolated, tmp_path):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "x",
...
def test_cli_spend_for_one_task(isolated):
    _config, _agent, t = _one_task()
    result = runner.invoke(app, ["sarsi", "spend", "--agent", "sarsi-worker",
                                 "--task", t.id])
    assert result.exit_code == 0
...
def test_cli_decisions_lists_what_the_agent_did_alone(isolated):
    ...
    ledger.append(config, "reports",
                  {"agent": "sarsi-worker", "task": "tsk_1", "state": "answered",
...
def test_cli_decisions_ack_moves_the_line(isolated):
    ...
    runner.invoke(app, ["sarsi", "decisions", "--agent", "sarsi-worker", "--ack"])
    result = runner.invoke(app, ["sarsi", "decisions", "--agent", "sarsi-worker"])
    assert "nothing decided without you" in result.output.lower()
...
def test_cli_blast_reports_the_declared_paths(isolated):
    _config, _agent, t = _one_task()
    result = runner.invoke(app, ["sarsi", "blast", "sarsi-worker", t.id])
    assert result.exit_code == 0
...
def test_cli_questions_lists_open_escalations(isolated):
    ...
    config = reg.load()
    ledger.append(config, "reports",
                  {"agent": "sarsi-worker", "task": "tsk_1", "state": "question",
...
def test_cli_answer_refuses_when_the_answer_would_reach_nobody(isolated):
    ...
    config, agent, t = _one_task()
    ledger.append(config, "reports",
                  {"agent": "sarsi-worker", "task": t.id, "state": "question",
                   "evidence": ["Q: which directory?", "escalated: x"]})
    result = runner.invoke(app, ["sarsi", "answer", "sarsi-worker", t.id,
...
def test_cli_attention_reports_a_terminal_no_task_claims(isolated, monkeypatch):
    ...
    from ai4science.harness.agents.sarsi import entry
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    monkeypatch.setattr(entry, "_live_names", lambda: {"sarsi-worker-9zz9"})
    result = runner.invoke(app, ["sarsi", "attention"])
    assert "sarsi-worker-9zz9" in result.output
...
def test_cli_undo_names_the_last_act_and_refuses_to_recall_mail(isolated):
    ...
    ledger.append(config, "outward",
                  {"agent": "sarsi-worker", "task": "tsk_1", "kind": "mail",
                   "destination": "them@example.com", "digest": "abc123",
                   "chars": 120, "outcome": "sent"})
    result = runner.invoke(app, ["sarsi", "undo", "sarsi-worker"])
    assert result.exit_code != 0
...
def test_cli_undo_shows_without_acting_when_asked(isolated):
    ...
    config = reg.load()
    ledger.append(config, "outward",
                  {"agent": "sarsi-worker", "task": "tsk_1", "kind": "post",
                   "destination": "mastodon", "digest": "d1", "chars": 40,
                   "outcome": "posted"})
    result = runner.invoke(app, ["sarsi", "undo", "sarsi-worker", "--show"])
    assert result.exit_code == 0
...
def test_cli_do_can_declare_a_budget(isolated, tmp_path):
    from ai4science.harness.agents.sarsi import task as tsk
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "index the exports",
                                 "--steps", "40", "--minutes", "30"])
    assert result.exit_code == 0
    config = reg.load()
    t = tsk.all_of(config, config.agents["sarsi-worker"])[0]
    assert (t.max_steps, t.max_minutes) == (40, 30)


def test_cli_do_says_what_the_budget_is(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "x", "--minutes", "30"])
...
def test_cli_do_refuses_a_budget_of_zero(isolated):
    ...
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "do", "sarsi-worker", "x", "--steps", "0"])
    assert result.exit_code != 0


def test_cli_handoff_writes_and_shows_it(isolated):
    _config, _agent, t = _one_task("tidy the report folder")
    result = runner.invoke(app, ["sarsi", "handoff", "sarsi-worker", t.id])
    assert result.exit_code == 0
    assert "tidy the report folder" in result.output
...
def test_cli_do_can_declare_a_dependency(isolated):
    from ai4science.harness.agents.sarsi import task as tsk
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "do", "sarsi-worker", "produce the numbers"])
    config = reg.load()
    first = tsk.all_of(config, config.agents["sarsi-worker"])[0]
    result = runner.invoke(app, ["sarsi", "do", "funding", "use the numbers",
                                 "--after", f"sarsi-worker/{first.id}"])
    assert result.exit_code == 0
...
def test_cli_rules_adds_and_lists(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "rules", "sarsi-worker", "--add",
                        "use python3 on this host, never python"])
    result = runner.invoke(app, ["sarsi", "rules", "sarsi-worker"])
    assert result.exit_code == 0
...
def test_cli_rules_refuses_a_secret_value(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "rules", "sarsi-worker", "--add",
...
def test_cli_rules_can_remove_one(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "rules", "sarsi-worker", "--add", "use python3"])
    runner.invoke(app, ["sarsi", "rules", "sarsi-worker", "--remove", "use python3"])
    result = runner.invoke(app, ["sarsi", "rules", "sarsi-worker"])
...
def test_cli_who_suggests_a_worker(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "who", "open the editor and fix the header"])
    assert result.exit_code == 0
    assert "sarsi-worker" in result.output and "editor" in result.output
...
def test_cli_handoff_proposes_and_accepts(isolated):
    ...
    proposed = runner.invoke(app, ["sarsi", "handoff", "sarsi-worker", t.id,
                                   "--to", "funding",
...
def test_cli_board_writes_a_page_without_serving(isolated, tmp_path):
    ...
    result = runner.invoke(app, ["sarsi", "board", "sarsi-worker", "--write", str(out)])
    assert result.exit_code == 0
...
def test_cli_board_written_without_an_agent_is_the_index(isolated, tmp_path):
    ...
    text = out.read_text()
    assert "sarsi-worker" in text and "social" in text
...
def test_cli_publish_and_read_the_shared_tier(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    runner.invoke(app, ["sarsi", "publish", "sarsi-worker", "deadline",
...
def test_cli_publish_refuses_a_host_fact(isolated):
    runner.invoke(app, ["sarsi", "init", "--owner-id", "7007143162"])
    result = runner.invoke(app, ["sarsi", "publish", "sarsi-worker", "note",
