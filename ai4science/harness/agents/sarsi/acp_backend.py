...
WHICH ACP MODULE TO USE (this file is the *backend*, sibling `acp.py` is the
*transport*):

    Use `acp_backend` (this module) when you START a governed, single-turn
    session: you need the four-outcome verdict (ANSWERED / REFUSED / ERRORED /
    SILENT), the PreToolUse governance hook written BEFORE the peer spawns, the
    agent command+argv resolved from `openclaw.json`, or the `spawn` report that
    tells running / finished / never_started / unknown apart.

    Use `acp` (the sibling) when you want a PERSISTENT, RESUMABLE connection
    keyed by launch command — a cached runtime that survives a gateway restart
    via `resume`, driving `openclaw acp`, `ai4science acp` or bare `opencode`.

Rule of thumb: verdict + governance for one turn -> here; a long-lived session
you re-open after the gateway dies -> `acp`. Neither re-attaches to a live peer
across OS processes; that needs the gateway session API and is not implemented
in either.
