...
RESEARCH_PROMPT = (
    ...
    "have the generic coding tools AND read-only access to the full PWM registry "
    "stack (L1→L4): pwm_search (keyword-search principles+digital-twins+benchmarks "
    "by topic — use this first), pwm_overview (the landscape), "
    "pwm_principles/pwm_principle (L1 — a principle's detail also lists its digital "
    "twins + benchmarks), pwm_specs/pwm_spec (L2 — the digital-twin forward-model "
    "setups: six_tuple, protocol_fields, d_spec), pwm_benchmarks/pwm_benchmark (L3), "
    "and pwm_solutions (L4 — the registered SOTA leaderboard: solutions + their "
    "scores per benchmark). Mainnet/testnet status is each artifact's chain_status.\n\n"
    ...
    "the relevant pwm_principles/pwm_specs/pwm_benchmarks, and ALWAYS pwm_solutions "
    "for the current best registered result "
