...
55.9% of the ceiling, at AUC 0.850, and EF@1% 39.2 on the two targets held out
entirely** on seed 42, and passes. The held-out-target figure is inside the
headline because it is the harder claim, not because it is the better number: on
this seed it is below the overall figure, and every run report now leads with
both.
...
|---|---|---|---|---|
| 1 | **Series-disjoint splits** | `query-to-test analogue similarity is below 0.1 and the enrichment survives` | actives in standard screening sets are analogue series. A random split shows the model relatives of what it is scored on, and it then measures memory rather than recognition. Every enrichment number computed before this is fixed is uninterpretable | partly — the split is repaired; the rung is not closed. Measured 2026-08-11 and re-measured 2026-08-12 on the same corpus, like for like on seed 42: raw analogue similarity fell 0.526 → 0.261, the decoy-subtracted gap fell 0.372 → 0.110, AUC 0.955 → 0.850. The task got harder, which is how you know it was repaired and not loosened. But the gap runs 0.086 to 0.159 across twelve seeds, mean 0.116, so it clears this rung's own 0.1 bar on 5 of the 12 — a minority, and raw similarity clears it on none of them. `done` was this page claiming a rung its own measurement does not close. Re-arguing the 0.1 bar would be a re-basing, which is the panel's to decide and not this agent's |
...
| 4 | **Held-out targets, not just held-out molecules** | `performance is reported on targets absent from fitting, separately from held-out molecules` | generalising to a new compound in a known pocket is a different claim from generalising to a new pocket, and only the second one is useful | partly — the prominence half is done, 2026-08-12: every run report now leads with the objective and its declared guardrails on one line, `ef_at_1pct_heldout_targets` among them, and the page's headline sentence, the `L4 · solution` row and the README index row all carry it beside the overall EF. It stays a guardrail and does not become the objective — the objective is the metric, and this agent may not move it. Whether that closes the rung is the registry's to say: this agent proposes, it does not close |
...
| **L4 · solution** | EF@1% of **33–56, which is 45–77% of the achievable ceiling and 1.5×–3.5× the property baseline**, with **held-out-target EF@1% of 45–69** beside it, across twelve seeds, measured 2026-08-11 and re-measured 2026-08-12. Four numbers because the first one alone would have been unreadable, and because generalising to a new pocket is the claim a reader wants |
