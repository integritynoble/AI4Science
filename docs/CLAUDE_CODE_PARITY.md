...
**Config** — the gate is active only when the **PWM funding route** is selected **and** a
`pwm_…` token is present (`PWM_TOKEN` or `PWM_ONBOARD_TOKEN`). The route is decided by
selection, never by identity (`ai4science/funding.py`): `AI4SCIENCE_FUNDING=own|pwm`,
`AI4SCIENCE_PWM_GATE=1`, or `ai4science funding own|pwm`; otherwise an own-LLM login,
stored key or provider env key means the **own** route (0 PWM, no login, no balance
read, never the proxy), and a PWM token with nothing else means **pwm**. The API base is
`PWM_BASE` / `PWM_ONBOARD_BASE` (default `physicsworldmodel.org`). Without a token the
gate is disabled, so dev and CI run free. A priced plug-in the user picks is billed on
its own selection (`PwmGate.for_selected_service`), on either route.
...
| **Read-only bash auto-allow** | ✅ all 3 | `find`/`ls`/`grep`/`cat`/`git log`… run WITHOUT a permission prompt, exactly like the product (v0.5.6). Conservative classifier (`is_read_only_bash`, `harness/permissions.py`), shell-aware since v0.5.7 (shlex tokenizer: quoted `\|`/`;` are data — `grep -E "a\|b"` auto-runs; unbalanced quotes reject): allowlisted binaries only, redirects only to `/dev/null`/fd-merges, no `$(…)`/backticks, `find` without `-exec`/`-delete`, read-only `git` subcommands, every pipe/`;`/`&&`/`<(…)` segment must qualify. Anything unprovable falls back to the confirm. Runs even in `/readonly` (plan-mode parity); protected-dir sandbox still wins |
| **`a(lways)` on the native confirm** | ✅ all 3 | native per-edit confirm is now `[y/N/a(lways)]` like the SDK path — `a` allows that tool name for the rest of the session (`make_confirm`, repl.py, v0.5.7) |
| **End-of-turn recap** | ✅ native | one-sentence LLM recap after substantial turns (≥2 tools or ≥20s crunch): dim `✶ recap: …` under the footer; one cheap low-reasoning call on the session brand, metered like any turn; failures silent. `AI4SCIENCE_RECAP=0` off / `always` every turn (`harness/recap.py`, v0.5.7) |
| **Esc interrupts the running turn** | ✅ native (full TUI) | Esc while busy → `harness/interrupt.py` event: a running bash is killed (whole process group) within ~200ms, remaining tool calls in the batch are answered as `[skipped — user interrupted]` (history stays API-valid), the turn ends with `[harness] turn interrupted by user (Esc)`, and queued input runs next. Fixes the "find / locks the TUI for 300s" hang (v0.5.8). Alt+Enter newline still works (non-eager chord) |
...
agent the product's visual shell. **The full-screen TUI is the DEFAULT on a
real terminal (v0.5.5)** — like the product itself; `AI4SCIENCE_TUI` tunes it:
