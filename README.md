...
(an instant reward that refills a shrinking runway — ~19 turns, then 18, …
floor 5 — unlocked only when your PWM is nearly used up after real usage),
...
```bash
# Pin an exact release — Linux/macOS (accepts "0.6.26" or "v0.6.26")
curl -fsSL https://raw.githubusercontent.com/integritynoble/AI4Science/main/install.sh | bash -s -- --version 0.6.26
...
$env:AI4SCIENCE_VERSION="0.6.26"; irm https://raw.githubusercontent.com/integritynoble/AI4Science/main/install.ps1 | iex
```
...
From source (to modify / run tests): `git clone … && pip install -e ".[dev]" && pytest`. That is the whole contract — `[dev]` alone must **collect** the entire suite with zero errors *and* **run** it green, and [`scripts/check-hermetic.sh`](scripts/check-hermetic.sh) proves the collect half from a fresh venv. `[dev]` therefore carries `claude-agent-sdk` and the eight first-party `pwm-agent-*` packages: around fifty tests assert on the agent registry those packages populate, and the specs they ship are deliberately not built in. Tests that need the `pwm_control_plane` sibling (not on PyPI) or podman skip rather than failing to import; [`scripts/dev-venv.sh`](scripts/dev-venv.sh) builds the fuller environment — `[dev]` plus any sibling checkouts (`PWM_SIBLINGS=…`, default: this repo's parent) installed editable — that turns those skips into coverage, and tells you whether the `claude` CLI is on PATH. Full step-by-step for any OS incl. Windows: [`docs/INSTALL.md`](docs/INSTALL.md). Maintainers: [`docs/RELEASING.md`](docs/RELEASING.md).
...
**Worked example:** [`examples/draft_principle`](examples/draft_principle) — a full four-layer PWM contribution (1D heat equation) drafted conversationally: L1 Principle → L2 Spec → L3 Benchmark → L4 Solution, ending in a runnable Crank-Nicolson solver that beats the benchmark threshold (~140×). Every artifact is agent-written, schema-valid, and the solver pipeline is reproducible.
